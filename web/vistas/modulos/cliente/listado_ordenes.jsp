    ...
    <body>
        ...
        <main class="bg-secondary text-dark">
            ...
            <section class="container border bg-light">                
                ...
                <c:if test = "${cant_lista_ordenes_compra == false}">
                    <div class="form-row bg-light text-center">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                <thead>
                                    <tr>                                    
                                        <th scope="col">Orden número</th>
                                ...
                                <tbody>
                                    <c:forEach var="listaOC" items="${listado_orden_compra}">                          
                                        <tr>                                       
                                            <td>${listaOC.getId()}</td>
                                            <td>${listaOC.getFecha()}</td> 
                                            <td>${listaOC.getCant_huespedes()}</td>
                                            <td>${listaOC.getCant_habitaciones()}</td> 
                                            <td>${listaOC.getFecha_ingreso()}</td> 
                                            <td>${listaOC.getFecha_salida()}</td> 
                                            <td>${listaOC.getNombre_menu()}</td>                                            
                                            <td>$${listaOC.getMonto_total()}</td>
                                            <td>
                                                <form method="GET" action="/Login_Hostal/EliminarReserva">
                                                    <button type="submit" name="btn_eliminar_oc" class="btn btn-danger" value="${listaOC.getId()}">Eliminar</button>
                                                </form>
                                            </td>                                            
                                        </tr>
                                    </c:forEach>
                ...
                <c:if test = "${cant_lista_ordenes_compra == true}">
                    <div class="largo_ventana">
                        <h1 class="alert alert-info">No existen órdenes asociadas a su número de cliente.</h1>
                    </div>
                </c:if>               
            </section>            
        </main>
