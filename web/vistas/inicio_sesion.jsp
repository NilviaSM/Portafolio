...
<html lang="es-ES">    
    ...
    <body>        
        ...
        <main class="fondo-principal salto-linea">
            <section class="fondo-inicio-sesion container text-light bg-secondary align-items-center">
                <h1 class="display-3 text-center pruebas43">
                    Iniciar sesión
                </h1>
                <form class="py-5 px-5" action="/Login_Hostal/ControladorLogin" method="post">
                    <div class="form-group">
                        <label for="correo_electronico">Usuario</label>
                        <input type="text" class="form-control" id="correo_electronico" name="correo_electronico" aria-describedby="emailHelp" required="true" minlength="4" placeholder="usuario@dominio.com">                        
                    </div>
                    <div class="form-group">
                        <label for="contrasena">Contraseña</label>
                        <input type="password" class="form-control" id="contrasena" name="contrasena" required="true" minlength="3" placeholder="contraseña">
                    </div>
                    <div class="form-group form-check">
                        <input type="checkbox" class="form-check-input" id="exampleCheck1">
                        <label class="form-check-label" for="exampleCheck1">Recordarme</label>
                    </div>
                    <c:set var = "verificar_sesion" value = "${validarInicioSesion}"/>
                    <c:if test = "${verificar_sesion == true}">                    
                        <div class="py-3 mb-2 px-3 alert-danger"><c:out value = "${validaSesion}"/></div>                                         
                    </c:if>
                    <input  type="submit" name="btn_entrar"  value="Iniciar sesión" class="btn btn-warning py-3 px-5" >
                </form>
            </section>            
        </main>       
        <footer class="footer-principal text-center text-light bg-dark">
