<%-- 
    Document   : listar_empleado
    Created on : 06-10-2020, 16:14:17
        ...
        <main class="fondo-principal text-light salto-linea">
            <section class="container border bg-secondary">
                ...
                <c:if test = "${valida == true}">           
                    ...
                    <form action="/Login_Hostal/ListarEmpleados" method="POST" accept-charset=utf-8>
                        <div class="form-group col-lg-12 my-3 text-right">                                                               
                        </div>
                    </form>
                </c:if>
                
                <c:if test = "${cant_lista_empleado == false}">
                <div class="my-3">                    
                    <table class="table bg-light pt-2">
                        ...
                        <tbody>
                            <c:forEach var="listaEMP" items="${listado_empleado}">
                                <tr> 
                                     <td><c:out value="${listaEMP.nombre}" /></td>
                                    <td>${listaEMP.rut}</td> 
                                    <td>${listaEMP.userName}</td>
                                    <td>${listaEMP.nombre_contacto}</td>
                                    <td>${listaEMP.correo_electronico}</td>
                                    <td>${listaEMP.telefono}</td>
