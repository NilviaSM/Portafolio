<%-- 
    Document   : listar_proveedor
    Created on : 06-10-2020, 15:42:23
        ...
        <main class="fondo-principal text-light salto-linea">
            <section class="container border bg-secondary">
                ...
                <c:if test = "${valida == true}">           
                    ...
                    <form action="/Login_Hostal/ListarProveedor" method="POST" accept-charset=utf-8>
                        <div class="form-group col-lg-12 my-3 text-right">                                                               
                        </div>
                    </form>
                </c:if>
                <c:if test = "${cant_lista_proveedor == false}">
                    <div class="my-3">                    
                        <table class="table bg-light pt-2">
                            ...
                            <tbody>
                                ...
                                <c:forEach var="listaPRO" items="${listado_proveedor}">
                                    <tr>                                
                                        <td><c:out value="${listaPRO.rubro}" /></td>
                                        <td>${listaPRO.nombre}</td>
                                        <td>${listaPRO.rut}</td> 
                                        <td>${listaPRO.userName}</td>           
                                        <td>${listaPRO.telefono}</td> 
                                        <td>${listaPRO.email}</td> 
                                        <td>${listaPRO.nombre_contacto}</td>
                                        <td>
                                            <form method="GET" action="/Login_Hostal/EliminarProveedor">
                                                <button type="submit" name="btn_eliminar_PRO" class="btn btn-danger" value="${listaPRO.id}">Eliminar</button>
                                            </form>
                                        </td>                                     
                                    </tr>
                                </c:forEach>
